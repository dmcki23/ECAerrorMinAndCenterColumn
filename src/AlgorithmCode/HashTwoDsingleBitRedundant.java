    ...
    public int[][] initializeDepthZero(int[][] input, int rule, boolean minimize, boolean rowError) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                for (int r = 0; r < 2; r++) {
                    for (int c = 0; c < 2; c++) {
                        cell += (int) Math.pow(16, 4 * r + c) * output[0][(row + r) % rows][(col + c) % cols];
                    }
                }
                //finds the neighborhood's codeword
                //output[1][row][col] = hashTruthTables.minSolutionsAsWolfram[rule][cell];
...
//        }
    ...
    public int[][] inverseRedundancy(int[][] input){
        int[][] out = new int[input.length][input[0].length];
        for (int row = 0; row < input.length; row++) {
            for (int col = 0; col < input[0].length; col++) {
                for (int power = 0; power < 4; power++) {
                    if ((out[row][col]>>power)%2 == 0){
                        out[row][(col+power)%input[0].length] += 1;
                    } else {
                        out[row][(col+power)%input[0].length] -= 1;
                    }
                }
...
//        System.out.println("done with gif");
    ...
    public void verifyInverseAndAvalancheSingleBits(String filepath) throws IOException {
        ...
        int[][] tempAB;
        for (int t = 0; t < 32; t++) {
            tempB = new int[bFieldSet[0].length][bFieldSet[0][0].length];
            tempAB = new int[bFieldSet[0].length][bFieldSet[0][0].length];
            for (int row = 0; row < bFieldSet[0].length; row++) {
                for (int column = 0; column < bFieldSet[0][0].length; column++) {
                    for (int offset = 0; offset < 4; offset++){
                        tempB[row][column] += (1<<offset)*(bFieldSet[t][row][(column+offset)%bFieldSet[0][0].length]);
                        tempAB[row][column] += (1<<offset)*(abbFieldSet[t][row][(column+offset)%bFieldSet[0][0].length]);
                    }
                    //abbFieldSet[t][row][column] = bFieldSet[t][row][column];
                }
            }
            for (int row = 0; row < bFieldSet[0].length; row++) {
                for (int column = 0; column < bFieldSet[0][0].length; column++) {
                    bFieldSet[t][row][column]  = tempB[row][column];
                    abbFieldSet[t][row][column]  = tempAB[row][column];
                }
            }
        }
        ...
        for (int d = 1; d <= depth; d++) {
            ...
            for (int t = 0; t < 32; t++) {
                ...
                individualDifferences = inverseRedundancy(individualDifferences);
                for (int row = 0; row < individualDifferences.length; row++) {
                    for (int col = 0; col < individualDifferences[0].length; col++) {
                        for (int power = 0; power < 1; power++) {
                            //sum the discrepancies
            ...
            setInverse = inverseRedundancy(setInverse);
            for (int row = 0; row < setInverse.length; row++) {
                for (int col = 0; col < setInverse[0].length; col++) {
                    for (int power = 0; power < 1; power++) {
                        //sum the discrepancies
        ...
        for (int d = 0; d <= depth; d++) {
            System.out.println("avalancheDifferences: " + Arrays.toString(avalancheDifferences[d]));
        }
//        //int[] betterThanHalf = new int[32];
//        for (int t = 0; t < 32; t++) {
//            listIndex = (t / 16) % 2;
