...
public class HashTwoDsingleBit {
     ...
     * the rest is empty
     */
    public int[][][] initializeDepthZeroRed(int[][][] input, int rule, boolean minimize, boolean rowError) {
...
//        }
     ...
     * @return inverted hashed data
     */
    public int[][] inverseHexRed(int[][][] input, int depth, int rule, boolean minimize, boolean rowError) {
        int neighborDistance = 1 << (depth - 1);
        //neighborDistance = 1;
     ...
     * @return inverted hashed data
     */
    public int[][] inverseRed(int[][][] input, int depth, int rule, boolean minimize, boolean rowError) {
        int neighborDistance = 1 << (depth - 1);
        //neighborDistance = 1;
        int[][][] votes = new int[input.length][input[0].length][1];
        int minimizer = (minimize ? 0 : 1);
...
//        }
     ...
     * @return inverted hashed data
     */
    public int[][] inverseHexRed(int[][][][] input, int depth) {
        int neighborDistance = 1 << (depth - 1);
        //neighborDistance = 1;
     ...
     * @return inverted hashed data
     */
    public int[][] inverseRed(int[][][][] input, int depth) {
        int neighborDistance = 1 << (depth - 1);
        //neighborDistance = 1;
...
//        //System.out.println(Arrays.toString(allTables[3][165]));
    ...
    public void verifyInverseAndAvalancheSingleBitsRedundant(String filepath) throws IOException {
        ...
        for (int posNeg = 0; posNeg < 32; posNeg++) {
            minimize = (posNeg / 8) % 2 == 0 ? true : false;
            rowError = (posNeg / 16) % 2 == 0 ? true : false;
            bFieldSet[posNeg] = initializeDepthZeroRed(bFieldSet[posNeg], hash.bothLists[(posNeg / 16) % 2][posNeg % 8], minimize, rowError);
            abbFieldSet[posNeg] = initializeDepthZeroRed(abbFieldSet[posNeg], hash.bothLists[(posNeg / 16) % 2][posNeg % 8], minimize, rowError);
        }
        System.out.println(bFieldSet.length + " " + bFieldSet[0].length + " " + bFieldSet[0][0].length + " " +  bFieldSet[0][0][0].length);
        //make a copy of the initial bitmap raster breakdown for later comparison to hashed versions
        for (int t = 0; t < 32; t++) {
        ...
        int[] betterThanHalf = new int[32];
            ...
            for (int d = 1; d <= depth; d++) {
                ...
                for (int t = 0; t < 32; t++) {
                    //for every cell of inverses
                    //single bit version
                    int[][] individualDifferences = inverseRed(hashedRearranged[d][t], d, hash.bothLists[(t / 16) % 2][t % 8], (t / 8) % 2 == 0 ? true : false, (t / 16) % 2 == 0 ? true : false);
                    ...
                    //hex version
                    individualDifferences = inverseHexRed(hashedRearranged[d][t], d, hash.bothLists[(t / 16) % 2][t % 8], (t / 8) % 2 == 0 ? true : false, (t / 16) % 2 == 0 ? true : false);
                        System.out.println(individualDifferences.length + " " + individualDifferences[0].length);
                    for (int row = 0; row < individualDifferences.length; row++) {
                        for (int col = 0; col < individualDifferences[0].length; col++) {
                            for (int power = 0; power < 4; power++) {
                ...
                //for the single bit entire codeword set inverse
                int[][] setInverse = inverseRed(hashedRearranged[d], d);
                for (int row = 0; row < setInverse.length; row++) {
                    for (int col = 0; col < setInverse[0].length; col++) {
                        //sum the discrepancies
                        //sum the 1 bits to verify that the algorithm is doing anything at all
                ...
                //for the hex entire codeword set inverse
                setInverse = inverseHexRed(hashedRearranged[d], d);
                for (int row = 0; row < setInverse.length; row++) {
                    for (int col = 0; col < setInverse[0].length; col++) {
                        for (int power = 0; power < 4; power++) {
                            //sum the discrepancies
